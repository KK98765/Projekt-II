...
def natural_cubic_spline(nodes):
    ...
    for i in range(n):
        ...
        B[i] = z[i] / 2
        C[i] = - (h[i] / 6 * (z[i+1] + 2 * z[i]) + 1 / h[i] * (nodes[i+1, 1] - nodes[i, 1]))
...
def compare_cubic_splines(nodes):
...
# Zad.1: Wykres funkcji sklejanej sześciennej
...
plt.plot(data[:, 0], data[:, 1], 'ro', label='Data Points')
plt.plot(x_interpolation_zad2, S_interpolation, label='Cubic Spline')
